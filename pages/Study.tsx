...
// Pastikan getDecks sudah ada di storage.ts, jika belum lihat langkah ke-2 di bawah
import { getCards, updateCard, getDecks } from '../utils/storage';
import { calculateSM2 } from '../utils/sm2';
import { Card, Grade } from '../types'; // Pastikan tipe Deck juga di-import jika ada
import { RefreshCw, Check, Keyboard, Zap, Layers } from 'lucide-react';

// Interface tambahan jika belum ada di types.ts
interface Deck {
  id: string;
  name: string;
}
...
export const Study: React.FC = () => {
  ...
  useEffect(() => {
    const loadQueue = () => {
      ...
      // PERBAIKAN 1 & 2: Panggil getCards() tanpa argumen, lalu filter manual
      const rawCards = getCards(); 
      
      // Filter berdasarkan deckId jika ada
      // Asumsi: object Card memiliki properti 'deckId'
      const allCards = deckId 
        ? rawCards.filter((c: any) => c.deckId === deckId) 
        : rawCards;
      
      // PERBAIKAN 3: Get deck name
      if (deckId) {
        try {
          const decks = getDecks(); // Pastikan fungsi ini me-return array
          // Explicit type 'any' atau 'Deck' untuk menghindari error implicit any
          const d = decks.find((d: any) => d.id === deckId);
          if (d) setDeckName(d.name);
        } catch (error) {
          console.warn("Gagal memuat nama deck, mungkin getDecks belum diimplementasi", error);
        }
      } else {
